...
struct BubbleMisconceptionView: View {
    ...
    @State private var isScrollEnabled = true // Add a variable to control scrolling
    ...
    var body: some View {
        ZStack{
            ...
            ScrollView ([.horizontal, .vertical], showsIndicators: false) {
                GeometryReader { geometry in
                    ...
                    // Include the background image as the first child of the ScrollView's content
                    //                                let backgroundImage = Image("WomenAreTheFutureEdit")
                    //                                    .resizable()
                    //                                    .scaledToFill()
                            ...
                            .gesture(
                                DragGesture(minimumDistance: 1)
                                    .updating($isDragging) { value, state, _ in
                                        state = true // Update the gesture state to true when dragging
                                    }
                                    .onChanged { value in
                                        // Check if the drag movement is slicing
                                        if smashState {
                                            withAnimation {
                                                isExploded.toggle()
                                            }
        ...
        .environment(\.isEnabled, isScrollEnabled) // Control scrolling with environment
        .simultaneousGesture(
